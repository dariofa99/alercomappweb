  ...
  export class AlertAdapter implements Adapter<AlertModel> {
    adapt(item: any): AlertModel {
      return new AlertModel(item.id,item.event_description, item.event_date,
        ...
        item.affected_livelihoods,new EventTypeModel(item['event_type'].id,item['event_type'].event_type_description,item['event_type'].event_type_name,item['event_type'].category_id),item.event_type_id,item.town_id,item.status_id,item.afectations_range_id,
