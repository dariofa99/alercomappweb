...
export class TypealertsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'event_type_name', 'event_type_description', 'category', 'action'];
  displayedColumnsSpanish: string[] = ['ID', 'Nombre', 'Descripción', 'Categoría','Acción'];
  ...
  constructor(private auth: AuthService, private references: ReferencesService,
    ...
    private toastr: ToastrService, private eventTypeService: EventypesService) { 
  ...
  ngOnInit(): void {
    this.eventTypes.forEach(element => {
      this.eventTypesToDisplay.push(new EventTypeToDisplay(element.id,element.event_type_name,
        element.event_type_description,element.category.reference_name))
    });
  ...
  editEventType(eventType: any){
    this.router.navigate(["/home/admin-event-types/edit-event-type/"+eventType.id]);
  }
  ...
  deleteEventType(id:number){
    Swal({  
      ...
      cancelButtonText: `No`,
        ...
        if (result.value) {    
          this.eventTypeService.deleteEventType(this.auth.readToken(),id).subscribe({
            next: data => {
              if(data['errors']!=undefined?data['errors'].length!=0:false){
                ...
                this.eventTypes = this.eventTypes.filter(item=>item.id!=id)
                this.eventTypesToDisplay = this.eventTypesToDisplay.filter(item=>item.id!=id)
                this.dataSource = new MatTableDataSource(this.eventTypesToDisplay);
              }
            },
            error: error => {
